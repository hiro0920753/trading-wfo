...
async function renderTradeChartV2(){
  ...
  try{
    ...
    const layout={
      paper_bgcolor:colors.panel,plot_bgcolor:colors.panel,font:{color:colors.text,size:13},
      margin:{l:78,r:35,t:40,b:55},height:Math.max(620,270*paneCount),
      hovermode:hoverValues?'x unified':false,hoverlabel:{bgcolor:colors.panel,bordercolor:colors.grid,font:{color:colors.text,size:13}},
      dragmode:'pan',showlegend:true,legend:{orientation:'h',y:1.025,x:0,font:{size:12}},shapes:[]
    };
    ...
    traces.push(
      ...
      {type:'scattergl',mode:'lines',name:'Open',x:sourceX,y:sourceRows.map(row=>row.open),...lineAxis,line:{color:colors.open,width:1.2,dash:'dot'}},
      {type:'scattergl',mode:'lines',name:'Close',x:sourceX,y:sourceRows.map(row=>row.close),...lineAxis,line:{color:colors.close,width:1.7}},
      {type:'scatter',mode:'markers+text',name:'ENTRY',x:[new Date(chartTime(trade.time)).toISOString()],y:[trade.entry_price],...lineAxis,text:['ENTRY'],textposition:'top center',textfont:{color:colors.entry,size:12},marker:{symbol:trade.side==='short'?'triangle-down':'triangle-up',size:17,color:colors.entry,line:{color:colors.panel,width:2}}},
      {type:'scatter',mode:'markers+text',name:'CLOSE',x:[new Date(chartTime(trade.exit_time)).toISOString()],y:[trade.exit_price],...lineAxis,text:['CLOSE'],textposition:'bottom center',textfont:{color:colors.exit,size:12},marker:{symbol:'x',size:16,color:colors.exit,line:{color:colors.exit,width:3}}}
    );
    ...
    for(let index=0;index<paneCount;index++){
      ...
      layout[xName]={domain:[0,1],anchor:axisRef('y',index),matches:index?'x':undefined,rangeslider:{visible:false},showticklabels:index===paneCount-1,gridcolor:colors.grid,linecolor:colors.grid,tickfont:{size:12,color:colors.text},zeroline:false,title:index===paneCount-1?{text:'Date / time',font:{size:13,color:colors.text}}:undefined};
      layout[yName]={domain:[bottom,top],anchor:axisRef('x',index),gridcolor:colors.grid,linecolor:colors.grid,tickfont:{size:12,color:colors.text},zeroline:false,title:{text:paneLabels[index]||'Value',font:{size:13,color:colors.text}}};
    }
    await Plotly.newPlot(plot,traces,layout,{responsive:true,scrollZoom:true,displaylogo:false,toImageButtonOptions:{format:'png',filename:`trade-${trade.position_id??'chart'}`}});
