...
function updateHoverMenuLabel(menu){menu.querySelector('[data-chart-command="hover"]').textContent=`${state.chartHoverValues?'✓':'　'} Show cursor values`}
function installTradeContextMenu(){const plot=document.querySelector('#trade-plot'),menu=document.createElement('div');menu.id='trade-chart-context-menu';menu.className='trade-context-menu hidden';menu.innerHTML='<button type="button" data-chart-command="save">Save this trade PNG</button><button type="button" data-chart-command="autoscale">Autoscale all axes</button><button type="button" data-chart-command="hover"></button>';document.body.append(menu);updateHoverMenuLabel(menu);plot.addEventListener('contextmenu',event=>{event.preventDefault();updateHoverMenuLabel(menu);menu.style.left=`${Math.min(event.clientX,window.innerWidth-220)}px`;menu.style.top=`${Math.min(event.clientY,window.innerHeight-150)}px`;menu.classList.remove('hidden')});menu.addEventListener('click',async event=>{const command=event.target.dataset.chartCommand;if(command==='save')saveTradeChart();if(command==='autoscale')autoscaleTradeChart();if(command==='hover'){state.chartHoverValues=!state.chartHoverValues;await renderTradeChartV2()}menu.classList.add('hidden')});document.addEventListener('click',event=>{if(!menu.contains(event.target))menu.classList.add('hidden')});window.addEventListener('scroll',()=>menu.classList.add('hidden'),true)}
